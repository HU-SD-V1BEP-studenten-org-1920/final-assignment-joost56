...
//
    ...
    @Path("{eigenaarid}")
    @Produces("application/json")
    public Response updateEigenaar(@PathParam("eigenaarid") int id, @FormParam("name") String name) {
    ...
    @Path("{eigenaarid}")
    @Produces("application/json")
    public Response updateEigenaarName(@PathParam("eigenaarid") int id, @FormParam("name") String name) {
        Eigenaar replaced = Eigenaar.updateEigenaarName(id, name);
        if(replaced==null) return Response.status(Response.Status.EXPECTATION_FAILED).entity(new AbstractMap.SimpleEntry<>("error", "kon klant" +id+ "niet updaten")).build();
        return Response.ok(Eigenaar.getEigenaar(id)).build();
    }
